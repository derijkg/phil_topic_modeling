...
def calculate_embedding_coherence(topic_word_lists, embedding_model):
    ...
    total_topic_coherence = 0.0
    for topic_words in topic_word_lists:
        if len(topic_words) < 2:
            continue # Cannot calculate coherence for a single word

        # Get embeddings for all words in the topic
        word_embeddings = embedding_model.encode(topic_words)

        # Calculate pairwise similarity
        similarity_matrix = cosine_similarity(word_embeddings)

        # Get the average of the upper triangle of the matrix, excluding the diagonal
        # This is the average pairwise similarity
        topic_coherence = np.mean(np.triu(similarity_matrix, k=1))
        total_topic_coherence += topic_coherence

    # Average the coherence over all topics
    return total_topic_coherence / len(topic_word_lists) if topic_word_lists else 0.0
...
def objective_function_with_data(trial, docs_list, embeddings_array, embedding_model_obj):
    ...
    else: # nr_topics_strategy Otherwise, encourage min_samples to be a bit larger to avoid instability
    ...
    log_params = (
        f"UMAP(nn={umap_n_neighbors}, nc={umap_n_components}, md={umap_min_dist:.2f}, m='{metric}'), "
        f"HDBSCAN(mcs={hdbscan_min_cluster_size}, ms={hdbscan_min_samples}, m='{metric}', csm='{hdbscan_selection_method}'), "
    ...
    try:
        fitted_vectorizer = topic_model.vectorizer_model
        analyzer = fitted_vectorizer.build_analyzer()
        logger.info("   Tokenizing docs for coherence with trial specific vectorizer...")
        tokenized_docs_for_gensim = [analyzer(doc) for doc in docs_list]
        # create dict
        gensim_dictionary = Dictionary(tokenized_docs_for_gensim)

        if bertopic_topics_words and gensim_dictionary:
            coherence_model_cv = CoherenceModel(
                topics=bertopic_topics_words,
                texts=tokenized_docs_for_gensim,
    ...
    coherence_embedding = calculate_embedding_coherence(bertopic_topics_words, embedding_model_obj)
...
if __name__ == "__main__":
    ...
    # using subset
    with open(r'..\data_processing\data_subset2.json','r') as f:
        subset_data = json.load(f)
    DOCS_FULL = subset_data

    if not DOCS_FULL: logger.error("No documents loaded."); exit()
    ...
    objective_with_fixed_data = partial(objective_function_with_data,
                                        ...
                                        embedding_model_obj = sentence_model_local)

    storage_name = "sqlite:///bertopic_paraphraph3_subcluster2.db" # New DB name
    study_name = "bertopic_paragraph3_subcluster2_study"
    ...
    if study.best_trial:
        ...
        final_metric = best_params.get('metric', 'cosine')
        final_umap_model = UMAP(n_neighbors=best_params['umap_n_neighbors'],
                                ...
                                metric=final_metric, random_state=42,
        ...
        final_topic_model = BERTopic(
            embedding_model=sentence_model_local, umap_model=final_umap_model, hdbscan_model=final_hdbscan_model,
        ...
        try:
            ...
            final_model_path = "final_bertopic_model_optimized_subcluster2"
